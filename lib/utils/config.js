...
import { watch, basePath, filesByExt, copyConfigSync, requireFileSync, existsSync, logger, } from 'node-karin';
const cacheList = {
 ...
 * @returns 布尔值
 */
export const setYaml = async (name, key, value) => {
 ...
 */
export const addYaml = (name, key, value) => {
    const file = path.join(dirConfig, `${name}.yaml`);
    if (!existsSync(file)) {
        logger.error(`${file} 不存在`);
        return false;
 ...
 * @returns 布尔值，操作成功返回 true，否则返回 false
 */
export const delYaml = (name, key, value) => {
    const file = path.join(dirConfig, `${name}.yaml`);
    if (!existsSync(file)) {
        logger.error(`${file} 不存在`);
        return false;
