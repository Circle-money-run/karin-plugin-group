...
export const blackWhiteList = karin.command(/^#(取消)?(拉黑|拉白)(群)?/, async (e) => {
    ...
    // 兜底
    if (!Array.isArray(cfg.group.disable_list))
        cfg.group.disable_list = [];
    if (!Array.isArray(cfg.user.disable_list))
        cfg.user.disable_list = [];
...
export const Botprefix = karin.command(/^#(添加|删除|查看)前缀/, async (e) => {
     ...
     * @param prefix 前缀
     */
    const checkPrefix = (prefix, alias) => {
    ...
    const handler = async (yamlKey, fileCfg) => {
        let key = fileCfg.key;
        ...
        if (e.msg.includes('添加')) {
            ...
            if (checkPrefix(prefix, cfg[key]?.alias || []))
                return true;
        ...
        if (e.msg.includes('删除')) {
            ...
            cfg[key].alias = cfg[key].alias.filter((item) => item !== prefix);
            config.setYaml(yamlKey, cfg);
            await e.reply('\n删除前缀成功', { at: true });
            return true;
        }
